...
export const generateWhatsAppReport = (
  ...
  const lines = [
    ...
    `Net Revenue (excl. GST):-${fmtMoney(toNum(r.net_revenue) || taxableRevenue)}`,
    '',
    '*Revenue Breakup*',
    '',
    ...
    ...(r.other_revenue_by_category ?? []).map((c) => `${c.category}:-${fmtMoney(c.amount)}`),
    ...
    ...(r.finance_expense_by_category ?? []).map((c) => `${c.category}:-${fmtMoney(c.amount)}`),
