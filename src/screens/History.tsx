...
import { aggregateRoomChart, calcOcc, fmtMoney, fmtInt, isStayOccupiedOnDate } from '@/lib/calc';
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-8">
      ...
      <main className="px-4 py-4 space-y-4 w-full">
        {error && (
          <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 text-rose-700 text-sm">{error}</div>
        )}

        {/* Filters */}
        <div className="bg-white rounded-xl border border-slate-200 p-3 flex gap-2 items-center">
          <div className="flex-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase">Year</label>
            <select
              value={year}
              onChange={(e) => setYear(Number(e.target.value))}
              className="w-full mt-0.5 border border-slate-200 rounded-lg px-2 py-1 text-sm bg-white"
            >
        ...
        {loading ? (
          ...
          <div className="space-y-2.5">
            {dates.map((dateStr) => {
              ...
              const agg = aggregateRoomChart(dayEntries, dateStr);
