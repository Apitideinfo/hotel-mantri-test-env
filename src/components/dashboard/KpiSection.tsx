...
const KpiCardItem = ({ label, value, sub, icon, color, iconBg, index }: KpiCardProps) => (
  <div
    className="bg-white rounded-2xl border border-slate-200/80 shadow-card hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-200 p-3.5 sm:p-4 flex flex-col justify-between gap-2 animate-kpi group min-w-0"
  ...
  >
    ...
    <div className="flex items-center justify-between gap-2">
      ...
      <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${iconBg} shrink-0 transition-transform group-hover:scale-105`}>
        {icon}
      </div>
    </div>
    ...
    <div className="min-w-0">
      <p className={`text-lg sm:text-xl xl:text-2xl font-bold tabular-nums leading-tight tracking-tight ${color}`} title={value}>
        {value}
      </p>
    </div>
