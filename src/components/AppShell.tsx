...
export const AppShell = ({ currentScreen, onNavigate, onSignOut, hotelName, posEnabled, children }: AppShellProps) => {
  ...
  const contentMarginClass = collapsed ? 'lg:ml-[72px]' : 'lg:ml-64';
  ...
  const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => {
    ...
    return (
      <>
        {/* Logo header */}
        <div className={`px-4 py-4 border-b border-white/10 flex items-center ${showLabels ? 'gap-2.5' : 'justify-center'}`}>
          <BrandIcon size={36} onDark />
          {showLabels && (
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-white truncate leading-tight">{hotelName ?? 'Hotel Mantri'}</p>
              <p className="text-[10px] text-brand-navy-300 uppercase tracking-wider">Management System</p>
            </div>
          )}
          {isMobile && (
        ...
        {showLabels && (
          <div className="px-3 pt-3">
            <div className="flex items-center gap-2 bg-white/5 rounded-lg px-2.5 py-1.5 border border-white/10">
              <Search className="w-3.5 h-3.5 text-brand-navy-300" />
              <input
                type="text"
                value={searchQuery}
        ...
        {/* Nav */}
        <nav className="flex-1 overflow-y-auto px-2.5 py-3 space-y-3 sidebar-scroll">
              ...
              <div key={group.label}>
                {showLabels && (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 mb-1.5 text-[10px] font-bold text-brand-navy-400 uppercase tracking-wider hover:text-brand-navy-200 transition"
                  >
                ...
                <div
                  ...
                  style={{
                    maxHeight: isGroupCollapsed ? '0' : '500px',
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative w-full flex items-center ${showLabels ? 'gap-2.5 px-3' : 'justify-center px-0'} py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                          active
                            ? 'bg-brand-600 text-white shadow-soft-blue'
                            : 'text-brand-navy-300 hover:text-white hover:bg-white/5'
                        }`}
                      >
                        {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 rounded-r-full bg-brand-gold-400" />}
        ...
        <div className="px-2.5 py-3 border-t border-white/10">
          <button
            ...
            className={`w-full flex items-center ${showLabels ? 'gap-2.5 px-3' : 'justify-center px-0'} py-2.5 rounded-lg text-sm font-medium text-brand-navy-300 hover:text-white hover:bg-red-900/30 transition-all`}
          >
