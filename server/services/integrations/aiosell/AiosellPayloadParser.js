...
export const parseWebhookPayload = (payload) => {
  ...
  return {
    ...
    roomCode: payload.roomCode || payload.room_code || null,
    rateplanCode: payload.rateplanCode || payload.rate_plan_code || payload.ratePlanCode || null,
    guestName: payload.guestName || payload.guest_name || (payload.guest ? `${payload.guest.firstName || ''} ${payload.guest.lastName || ''}`.trim() : 'Aiosell Guest'),
